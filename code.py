...
def dataProcesing(irList, redList, time, user):
    ...
    redfull = redList[0:300]
    redmid = redList[50:250]
    ...
    hrfull, hr_validfull, spo2full, spo2_validfull = calc_hr_and_spo2(irfull, redfull)
    hrmid, hr_validmid, spo2mid, spo2_validmid = calc_hr_and_spo2(irmid, redmid)
    ...
    if hr_valid3 != False and spo23 > 80 and spo23 > ox:
        hr = hr3
        ox = spo23
    if hr_validmid != False and spo2mid > 80 and spo2mid > ox:
        hr = hrmid
        ox = spo2mid
    if hr_validfull != False and spo2full > 80 and spo2full > ox:
        hr = hrfull
        ox = spo2full
    print(f"{spo21},{spo22},{spo23},{spo2mid},{spo2full}, {ox}")
    if hr == 0:
        hr = -1
        ox = -1
    else:
    ...
    print(f"hr: {hr}, spo2: {ox}, Mesage: {mesage}")
    return mesage
    # dataPlot(tupMAE, time)


def sendResult(s, result):
    s.connect(("192.168.4.1", 1337))
    result += "\r\n\r\n"
